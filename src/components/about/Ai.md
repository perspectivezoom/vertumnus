...
To be very clear: This project would never have gotten started without knowing Claude's capabilities beforehand, nor would I have completed this project without Claude's help (for some definitions of complete). The lowered friction of execution was instrumental in getting this to the finish line, especially dealing with historically persnickity bugbears like image placement and data processing. I had Claude attempt to curate a set of representative examples of Claude's prowess as [a transcript collection](/about/transcript/?view=collection%3Adiscovery), but I'm pretty sure the resulting list only makes sense to me. Oh well. Some notable wins that I would like to manually highlight, in order of importance descending:

### Switching from streamgraph to ridgeline charting

My original concept was a little more ambitious than the current version. I wanted to fill in all available vertical space in what Claude identified as a [streamgraph chart](https://en.wikipedia.org/wiki/Streamgraph). The streamgraph looked fine with fake data, but after populating the graph with real, spikier data, the streamgraph was far too wiggly to make any sense of the proportions. Claude [proposed a pivot to ridgeline](/about/transcript?view=topic%3Aridgeline&prompt=d0407555) instead, and after prototyping it out, I have to begrudingly admit, it's the right tool for the job. Claude also took care of all the D3 setup fiddliness that I remember not enjoying in the past.

### Suggesting and placing the USDA Pomological Watercolor Collection

After getting the ridgeline charts looking mostly correct, the poster gained significantly more whitespace and felt pretty sterile. I suggested some half baked ideas (unsplash b roll backgrounds, clip art, etc) and it [somehow suggested](/about/transcript?view=chapter%3Awatercolours&prompt=16ac61dc) the USDA Pomological Watercolor Collection, which was the vibe that I didn't know I wanted. While switching to ridgeline was the most important original insight, making use of the watercolor collection definitely feels the most inspired; it defines the look and feel of the poster as a whole.

### Finding and heuristically interpeting the USDA Market News Data

For San Francisco, I was searching for sub-month granularity produce seasonality. After a couple of dead ends, Claude discovered the USDA Market News shipping data. As a data source, it doesn't provide what I wanted out of the box. There are both outbound and inbound shipments, and it turned out that the [inbound shipments were less accurate](/about/transcript?view=topic%3Amars-movement&prompt=0a504642); San Francisco imports lots of produce from lots of different areas. Outbound shipments from regions surrounding San Francisco seemed to be the most accurate, and even then, Claude had to perform a fair amount of data massaging to eliminate one week gaps and smooth out spikes. I think, technically speaking, I could've discovered the data source and performed this step myself, but the amount of continuous iteration on the data would've taken me five times as long to execute, and I probably would have given up halfway through the process.
...
I have colleagues that have an AI agent spin off and write the code, commit, and even submit the PR (complete with AI playwright screenshots and video), but in my experience its code quality is not quite there yet for me to trust Claude Opus 5 to submit code unsupervised.

Similar to the Claude wins, I had Claude generate [a transcript collection](/about/transcript/?view=collection%3Apushback) of all the times that I was annoyed by its output, but I don't think it's new reader friendly. Some manual concrete examples of code quality misses, in order of prominence desc, but importance _asc_:
...
The other 50%, it seems to want to preserve Chesterton's Fence knowledge about some code decision that we made, except the decision is so obscure that it's irrelevant. For example, in reaction to an unimportant header id edge case, I had Claude toy around with different ways to render markdown, whereupon it discovered that the most obvious solution worked. It then proceeded [to comment that the edge case was the reason](/about/transcript?view=topic%3Awritten-markdown&prompt=1cf0fbc6). I mean, sure, header ids was the orgination trigger, but the edge case is not the main reason to go with the most standard way to render markdown. At best, this is something you write in a PR / commit message, for when you perform `git blame` code archeology.

### Component composition

My taste in React component architecture seems to be one level too strict, and I'm consistently telling Claude to [break up a component into child components](/about/transcript?view=topic%3Aribbons&prompt=8a7e7b17), and to convert the existing large component to an "orchestration" component, whose only job is to layout and render the child components. My poor brain can only load up so much context at once, and I'm constantly trying to "push complexity leafward" so that, from a readability perspective, I continually abstract away details that I don't need to think about at that component level. Claude seem to optimize for prototyping, where everything is in one gigantic component and it's easier to modify because concepts are more ephemeral.

Conversely, Claude is one notch too promiscuious with creating new files. Ask Claude to extract out a hook, and sometimes it will [make a new file](/about/transcript?view=topic%3Abrowser-contents&prompt=aef84f2c) instead of appending to the existing file, even if it is only ever consumed in that file. There's a real readability cost to having the code split up into an ocean of small files, and I think Claude undervalues the human discoverability of a long file with many related sibling components and helper functions.
...
For all my concerns, I am also very cognizant that my development workflow was completely different just three years ago, where everything was coded by hand. I am at least twice as productive as I was back then, and whatever arcane syntax knowledge that has atrophied, I have more than made up for it in terms of the number of problems I'm encountering and solving at the product level.
...
_Published September 2, 2026, Edited September 3_
